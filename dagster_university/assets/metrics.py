...
def trips_by_week(context: AssetExecutionContext, database: DuckDBResource) -> None:
    ...
    #     output_file.write(trips_by_week)
    trips_by_week.to_csv(constants.TRIPS_BY_WEEK_FILE_PATH)


@asset(
    deps=["taxi_trips"]
)
def trips_by_week2(database: DuckDBResource) -> None:
    ...
    result.to_csv(constants.TRIPS_BY_WEEK_FILE_PATH_2, index=False)